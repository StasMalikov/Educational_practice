...
$name=$_POST['name'];
$surname=$_POST['name_second'];
$patr=$_POST['name_patronic'];

require_once '../login.php';
$conn = new mysqli($hn, $user, $password, $database);
if ($conn->connect_error) die("Fatal Error");

$query  = "SELECT Id FROM Lecturers WHERE Name='$name' AND Surname='$surname' AND Patronymic='$patr'";
$result = $conn->query($query);

$row = $result->fetch_array(MYSQLI_ASSOC);
$lect_id=htmlspecialchars($row['Id']);
...
$query  = "INSERT INTO Lecturer_Subject VALUES(NULL,'$lect_id','$subj_id')";
$result = $conn->query($query);

$message='Преподаватель успешно записан.';
if (!$result) {
    $message='При записыввнии возникла ошибка: '. $conn->error;
}
...
<body>
    <div class='container-fluid'>
        ...
        <li class="list-inline-item"><a role="button" class="btn btn-info btn-lg"  href='add_subj_lect.php'>Запись преподавателя на предмет</a></li>
        </ul>
        <hr>
        ...
        <h4>Запись преподователя на предмет</h4>
